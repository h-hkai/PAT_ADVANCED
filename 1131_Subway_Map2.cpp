...
vector<int> graph[10005], past[10005],
    path;  //图、最短路径中每个结点的先驱结点、临时的最短路径
...
void DFS(int v) {  //深度优先遍历所有最短路径找出其中换乘站最少的路径
    path.push_back(
        v);  //结点编号压入最短路径中，注意遍历到起点时，整个路径是倒序存储在path中的
    if (v == start) {  //到达起点
        vector<pair<int, int>> tempTrans = {
            {start, -1}};  //当前最短路径所包含的换乘站
        for (int i = path.size() - 2; i > 0; --i)  //倒序遍历整个path
            if (line[path[i - 1] * 10000 + path[i]] !=
                line[path[i] * 10000 + path[i + 1]])  //当前结点是换乘站
                tempTrans.push_back(
                    {path[i], line[path[i] * 10000 + path[i + 1]]});
        tempTrans.push_back(
            {dest, line[path[1] * 10000 + dest]});  //将终点放入tempTrans中
        if (trans.empty() ||
            tempTrans.size() <
                trans.size())  // trans为空或tempTrans为换乘站更少的最短路径
...
void BFS() {                           //求解所有最短路径
    ...
    while (!q.empty()) {
        ...
        q.pop();
        if (dis[p] >
            dis[dest])  //当前结点的dis大于终点的dis，该结点一定不是最短路径中的结点
            continue;
